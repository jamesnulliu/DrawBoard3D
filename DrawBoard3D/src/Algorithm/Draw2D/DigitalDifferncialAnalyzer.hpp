...
class DigitalDifferencialAnalyzer : public Draw2D
{
public:
    explicit DigitalDifferencialAnalyzer() = default;
    ...
    virtual void Draw() const override final
    {
        ...
        f32 k = (m_end[1] - m_start[1]) / (m_end[0] - m_start[0]);
        f32 y = m_start[1];
        i32 i = 0;
        for (f32 x = m_start[0]; x <= m_end[0]; ++x) {
            if (m_mode == 0) {
